...
const encontrados = index.filter(item =>
item.conteudo && item.conteudo.toLowerCase().includes(pergunta)
)

if(encontrados.length > 0){

console.log("[DOC AI] Resultado encontrado em index.json:", encontrados.length)
...
const contexto = index.slice(-20).map(a=>
...
const resposta = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${process.env.GEMINI_KEY}`,{
...
const texto = data.candidates?.[0]?.content?.parts?.[0]?.text || "Não consegui encontrar resposta."
